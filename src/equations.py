...
def compute_z_deriv(phi, x, y, z, dir_id):
    ...
    dphi = np.zeros((nx+1,ny+1,nz+1))
    if dir_id == 0:
        dphi[:, :,     nz] = (phi[:, :,   nz] - phi[:, :,   nz-1]) / (z[:, :,   nz] - z[:, :,   nz-1])
        dphi[:, :, 0:nz-1] = (phi[:, :, 1:nz] - phi[:, :, 0:nz-1]) / (z[:, :, 1:nz] - z[:, :, 0:nz-1])
    if  dir_id == 1:
        dphi[:, :,      0] = (phi[:, :,    1] - phi[:, :,      0]) / (z[:, :,    1] - z[:, :,      0])
        dphi[:, :,   1:nz] = (phi[:, :, 1:nz] - phi[:, :, 0:nz-1]) / (z[:, :, 1:nz] - z[:, :, 0:nz-1])
    if dir_id == 2:
        dphi[:, :,      0] = (phi[:, :,    1] - phi[:, :,      0]) / (z[:, :,    1] - z[:, :,      0])
        dphi[:, :,     nz] = (phi[:, :,   nz] - phi[:, :,   nz-1]) / (z[:, :,   nz] - z[:, :,   nz-1])
        dphi[:, :, 1:nz-1] = (phi[:, :, 2:nz] - phi[:, :, 0:nz-2]) / (z[:, :, 2:nz] - z[:, :, 0:nz-2])
    return dphi
...
def Tauxy(U,V,x,y,z,mu,flux_dir,step):
    ...
    if step == 'predictor':
        if flux_dir == 0:
            tau_xy = mu * (compute_y_deriv(U,x,y,z,2) + compute_x_deriv(V,x,y,z,1))
        elif flux_dir == 1:
            tau_xy = mu * (compute_y_deriv(U,x,y,z,2) + compute_x_deriv(V,x,y,z,0))
        else:
        ...
        if flux_dir == 0:
            tau_xy = mu * (compute_y_deriv(U,x,y,z,1) + compute_x_deriv(V,x,y,z,2))
        elif flux_dir == 1:
            tau_xy = mu * (compute_y_deriv(U,x,y,z,0) + compute_x_deriv(V,x,y,z,2))
        else:
            raise Exception('Invalid Flux Direction')
...
def Tauxz(U,W,x,y,z,mu,flux_dir,step):
    ...
    if step == 'predictor':
        if flux_dir == 0:
            tau_xz = mu * (compute_z_deriv(U,x,y,z,2) + compute_x_deriv(W,x,y,z,1))
        elif flux_dir == 2:
            tau_xz = mu * (compute_z_deriv(U,x,y,z,2) + compute_x_deriv(W,x,y,z,0))
        else:
            raise Exception('Invalid Flux Direction')

    elif step == 'corrector':
        if flux_dir == 0:
            tau_xz = mu * (compute_z_deriv(U,x,y,z,1) + compute_x_deriv(W,x,y,z,2))
        elif flux_dir == 2:
            tau_xz = mu * (compute_z_deriv(U,x,y,z,0) + compute_x_deriv(W,x,y,z,2))
        else:
            raise Exception('Invalid Flux Direction')
...
def Tauyz(V,W,x,y,z,mu,flux_dir,step):
    ...
    if step == 'predictor':
        if flux_dir == 1:
            tau_yz = mu * (compute_z_deriv(V,x,y,z,2) + compute_y_deriv(W,x,y,z,1))
        elif flux_dir == 2:
            tau_yz = mu * (compute_z_deriv(V,x,y,z,2) + compute_x_deriv(W,x,y,z,0))
        else:
            raise Exception('Invalid Flux Direction')

    elif step == 'corrector':
        if flux_dir == 1:
            tau_yz = mu * (compute_z_deriv(V,x,y,z,1) + compute_x_deriv(W,x,y,z,2))
        elif  flux_dir == 2:
            tau_yz = mu * (compute_z_deriv(V,x,y,z,0) + compute_x_deriv(W,x,y,z,2))
        else:
            raise Exception('Invalid Flux Direction')
